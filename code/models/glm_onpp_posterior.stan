...
parameters {
  ...
  // vector[K-1] logratio_gamma;
  vector<lower=0>[K] aux_gamma;
}
transformed parameters {
  vector<lower=0, upper=1>[K-1] a0s;
  vector<lower=0, upper=1>[K] gamma;
  // vector<lower=0,upper=1>[K-1] loggamma;
  // loggamma = logratio_gamma - log1p_exp(logratio_gamma);
  // for (k in 1:(K-1)){
  //   a0s[k] = exp(log_sum_exp(loggamma[1:k]));
  // }
  gamma = aux_gamma / sum(aux_gamma);
...
model {
  ...
  if ( dist <= 2 ) {
    ...
    for ( k in 2:K ) {
      // target += logit_beta_lpdf(logratio_gamma[k-1] | alpha[k-1], alpha[K]);
