...
get.stan.data.onpp = function(
    ...
    disp.mean         = NULL,
    disp.sd           = NULL,
    alpha             = rep(1/length(data.list),
                            length(data.list)),
    get.loglik        = FALSE
) {
  hdbayes:::data.checks(formula, family, data.list, offset.list)
  
  res          = hdbayes:::stack.data(formula = formula,
                            data.list = data.list)
  y            = res$y
  X            = res$X
  ...
  # }
  # a0.lower = hdbayes:::to.vector(param = a0.lower, default.value = 0, len = K - 1)
  # if ( !is.null(a0.upper) ){
  ...
  # }
  # a0.upper = hdbayes:::to.vector(param = a0.upper, default.value = 1, len = K - 1)
...
glm.onpp = function(
  ...
  # )
  glm_npp_posterior = 
    cmdstanr::cmdstan_model("code/models/glm_onpp_posterior.stan")
